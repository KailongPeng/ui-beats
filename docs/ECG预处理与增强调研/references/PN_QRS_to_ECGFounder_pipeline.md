...
| 条件 | 判断 | 说明 |
|------|------|------|
| `mean(U_E + U_A) <= uc_thr` | ✓ 保留 | 模型自信 + 信号干净；默认阈值 `uc_thr=1.0` |
| 心拍数 5–25 个 / 10s | ✓ 保留 | 对应 30–150 bpm 正常范围 |
| 其中任一不满足 | ✗ 丢弃 | |

### 两种不确定性的含义

PN-QRS 在推理每个窗口时，模型内部同时计算：

| 不确定性 | 英文 | 含义 | 来源 |
|---------|------|------|------|
| **U_E 认知不确定性** | Epistemic Uncertainty | 模型对预测有多不自信 | `mi_est(logits)` |
| **U_A 偶然不确定性** | Aleatoric Uncertainty | 信号本身有多难判断 | `en_est(logits)` |

**关键**：此方法**只需要 CH20 单路信号**，不需要 12 导联参考。
内部实现：`uncertain_est()` 将 `eu[eu > 0.12] = 10` 二值化噪声帧，
所以电极脱落时的窗口 mean_uc 会被拉高到 ~10，与干净窗口 (~0.1–0.3) 差异悬殊，阈值容易区分。
